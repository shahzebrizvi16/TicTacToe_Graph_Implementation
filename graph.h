...
static const size_t MAXIMUM = _____
...
public:
// MEMBER CONSTANTS
static const std::size_t MAXIMUM = 9;
std::size_t edges[MAXIMUM][MAXIMUM]; // for testing purposes
...
// CONSTANT MEMBER FUNCTIONSvoid add_vertex(const Item& label);
std::size_t size( ) const { return many_vertices; }
std::size_t is_edge(std::size_t source, std::size_t target) const;
std::set<std::size_t> neighbors(std::size_t vertex) const;
Item operator[ ] (std::size_t vertex) const;


private:
//bool edges[MAXIMUM/3][MAXIMUM/3];
Item labels[MAXIMUM];
